...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/15 09:08:37 by jbashiri          #+#    #+#             */
/*   Updated: 2019/04/15 09:08:38 by jbashiri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

int				check_flags_two(const char *str, t_union *var)
{
	int			x;
	...
	while (argv[x] != 0)
	{
		if (ft_isdigit(argv[x]) || argv[x] == '-')
		{
			...
			if (argv[x] == '-' && !(argv[x + 1] >= '0' && argv[x + 1] <= '9'))
				return (-777);
			x += ft_power2(ft_atoi_with_err(argv + x, &err), 10);
			if (err == 1)
				return (-777);
			*size += 1;
		}
